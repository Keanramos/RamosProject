...
public class AddnewActivity2 extends AppCompatActivity {
            ...
            reference.child(passedName).addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    if (snapshot.exists()) {
                        ...
                        Toast.makeText(AddnewActivity2.this, "Existing user loaded.", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(AddnewActivity2.this, "No existing data. Please fill the form.", Toast.LENGTH_SHORT).show();
                    }
                }

                @Override
                public void onCancelled(@NonNull DatabaseError error) {
                    Toast.makeText(AddnewActivity2.this, "DB Error: " + error.getMessage(), Toast.LENGTH_SHORT).show();
                }
            });
        }
            ...
            if (!name.isEmpty() && !title.isEmpty() && !days.isEmpty() && !desc.isEmpty()) {
                ...
                reference.child(name).setValue(user).addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        Toast.makeText(AddnewActivity2.this, "Data saved successfully!", Toast.LENGTH_SHORT).show();
                        Intent intent = new Intent (AddnewActivity2.this, DashboardActivity.class);
                        startActivity(intent);
                    } else {
                        Toast.makeText(AddnewActivity2.this, "Failed to save data.", Toast.LENGTH_SHORT).show();
                    }
                });
            } else {
                Toast.makeText(AddnewActivity2.this, "Please fill all fields!", Toast.LENGTH_SHORT).show();
            }
        });
    }
}
